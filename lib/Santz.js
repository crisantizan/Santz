...
class Santz {
     ...
     * @param {number} fromOrCant Con dos parámetros: Número del registro desde el cual se va a empezar a mostrar. Con un parámetro: Cantidad de registros a mostrar.
     * @param {number} numRows Cantidad de registros a mostrar
     */
    limit(fromOrCant, numRows = null) {
        ...
        if (numRows === null) {
            query += `LIMIT ${fromOrCant}`;
        } else {
            query += `LIMIT ${fromOrCant}, ${numRows}`;
        }
        return this;
    }
    /**
