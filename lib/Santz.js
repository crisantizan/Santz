 ...
 * @param {object} poolC Conexión Pool establecida a MySQL.
 * @param {string} mode Manera en que la librería ejecutará sentencias.
 * @class Santz
 */
class Santz {

    constructor(poolC, strict = true) {
    ...
    /**
     * @description Convierte un string en código SQL ejecutable.
     * @param {string} strSql cadena de texto a ejecutar
     * @return string
    */
    strToSql(strSql) {
        return mysql.raw(strSql);
    }
    /**
     * @description Método que agrega la cláusula SELECT al query e inicializa la preparación de la sentencia SQL.
     * @param {array | string | object} param
     * @param {boolean} executable
     * @return Santz object
     */
    select (param, executable = false) {
        if (param) {
            if (executable && param.hasOwnProperty('toSqlString')) {
                query = 'SELECT ?';
                arrValues.push(param);
                return this;
            }
    ...
    from (table, staticTable = false) {
        ...
        if (typeof table === 'string' && table){
            ...
            if (!staticTable && strictMode) {
                firstColumnState = `${pool.escapeId(table)}.${pool.escapeId(delColumnName)} = 1`;
            }
        /*
    ...
    where (identifier, operator, value) {
        if (typeof identifier === 'undefined' || typeof operator === 'undefined' || typeof value === 'undefined') return null;
    ...
    hidden (table) {
        if (table && typeof table === 'string') {
            ...
            query = `UPDATE ?? SET ${pool.escapeId(table)}.${pool.escapeId(delColumnName)} = 0 `;
            arrValues.push(table);
            usedMethod = `hidden()`;
            return this;
        }
    ...
    show (table) {
        if (table && typeof table === 'string') {
            changeVisibility = true;
            query = `UPDATE ?? SET ${pool.escapeId(table)}.${pool.escapeId(delColumnName)} = 1 `;
            arrValues.push(table);
            usedMethod = `show()`;
            return this;
        }
    ...
    rowsHidden(table) {
        if (table && typeof table === 'string') {
            query = `SELECT * FROM ?? WHERE ${pool.escapeId(table)}.${pool.escapeId(delColumnName)} = 0`;
            arrValues.push(table);
    ...
    innerJoin (table, staticTable = false) {
        if (table && typeof table === 'string') {
            ...
            */
            if (!staticTable && strictMode) {
                arrColumnsState.push(`${pool.escapeId(`${table}.${delColumnName}`)} = 1`);
            }
            query += `INNER JOIN ?? `;
            arrValues.push(table);
            return this;
     ...
     * @param {string} column Nombre de la columna por la cual se ordenarán las filas.
    ...
    orderBy(column, mode='ASC') {
        if (column && typeof column === 'string') {
            order = ` ORDER BY ${pool.escapeId(column)} ${mode.toUpperCase()}`;
            return this;
        }
        return;
    }
    /**
     * @description Agrega la cláusula «LIMIT» a la consulta, usarse solo al final de esta. Recibirá dos parámetros cuando se quiera mostrar filas desde cierta posición hasta la cantidad deseada, o un solo parámetro cuando solo se quiera limitar la cantidad de registros a mostrar iniciando desde la primera posición.
     * @param {number} startOrAmount Con dos parámetros: Número del registro desde el cual se va a empezar a mostrar. Con un parámetro: Cantidad de registros a mostrar.
     * @param {number} numRows Cantidad de registros a mostrar
     */
    limit(startOrAmount, numRows = null) {
        /* Cuando solo se quiere limitar */
        if (numRows === null) {
            strLimitClausule = `LIMIT ${startOrAmount}`;
        } else {
            strLimitClausule = `LIMIT ${startOrAmount}, ${numRows}`;
        }
    ...
    exec () {
        const execution = new Promise( (resolve, reject) => {
            ...
            */
            if (isJoin && strLimitClausule || order && strLimitClausule) {
                /* Si no tiene un espacio al final, se le agrega */
                if (query[query.length - 1] !== ' ') {
                    query += ' ';
                }
            ...
            */
            ...
            // const result = pool.query(stmt, (err, rows, fields) => {
                // if (usedMethod && !strictMode) {
                //     return reject(`El método '${usedMethod}' solo se puede usar en modo estricto`);
                // }
