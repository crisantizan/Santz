...
export class Santz {
	...
	constructor(pool: Pool, strict: boolean = true) {
		_pool = pool;
		_strictMode = strict;
		_query = '';
		_delColumnName = '';
		...
		if (columns) {
			...
			if (columns.hasOwnProperty('toSqlString')) {
				...
				switch (executable) {
					...
					default:
						throw new Error('[Método select()]: para poder ejecutar código mediante el método «strToSql» se requiere el paso de un segundo parámetro con un valor de «true»');
				}
			}
			...
			if (typeof param === 'object' && !param.length) {
				...
				if(param.hasOwnProperty('all')) {
					...
					if (param.all === true) {
						...
						throw new Error('[Método select()]: Para seleccionar todas las filas en una consulta JOIN, debe pasar en un objeto la propiedad especial «_all» con un valor booleano «true»');
					}
				}
			...
			if ( (typeof columns === 'string' && columns === '*') || param[0] === '*') {
				_query = 'SELECT *';
				return this;
			}
			...
			throw new Error('[Método select()]: Debe pasar el o los nombres de las columnas a seleccionar');
		}
		return this;
	}
	public from (table: string, staticTable: boolean = false): this {
		...
		if (typeof table === 'string' && table) {
			...
			if (!staticTable && _strictMode) {
				_firstColumnState = `${_pool.escapeId(table)}.${_pool.escapeId(_delColumnName)} = 1`;
			}
			/*
			...
			*/
			...
			throw new Error('[Método from()]: Parámetro «table» es requerido y debe ser un string');
		}
		return this;
	}
	public where(identifier: string, operator: string, value: string | number): this {
		if (identifier === undefined || operator === undefined || value === undefined) {
		...
		*/
		...
		if (_haveWhere) {
			...
			*/
			if (!_changeVisibility && !_isDestroy && _firstColumnState && _strictMode) {
	...
	public insert(table: string, staticTable: boolean = false): this {
		if (table && typeof table === 'string') {
			if (staticTable && !_strictMode) {
				console.log(`[Método insert()]: PRECAUCIÓN: Cuando el modo estricto está inactivo, no es necesario definir una tabla como estática.\n`);
			}
			...
			throw new Error('[Método insert()]: El parámetro «table» es requerido y debe ser un string');
		}
	}
	public update(table: string, staticTable: boolean = false): this {
		if (table && typeof table === 'string') {
			if (staticTable && !_strictMode) {
				console.log(`[Método update()]: PRECAUCIÓN: Cuando el modo estricto está inactivo, no es necesario definir una tabla como estática.\n`);
			}
			...
			throw new Error('[Método update()]: El parámetro «table» es requerido y debe ser un string');
		}
	}
	public values(values: object): this {
		...
		if (newValues && typeof newValues === 'object') {
			...
			throw new Error('[Método values()]: El parámetro «values» debe ser un objeto y es requerido');
		}
	}
	public destroy(table: string): this {
		if (table && typeof table === 'string') {
			...
			throw new Error('[Método destroy()]: El parámetro «table» debe ser un string y es requerido');
		}
	}
	public hidden(table: string): this {
		if (table && typeof table === 'string') {
			_changeVisibility = true;
			_query = `UPDATE ?? SET ${_pool.escapeId(table)}.${_pool.escapeId(_delColumnName)} = 0 `;
			_arrValues.push(table);
			_usedMethod = `hidden()`;
			return this;
		} else {
	...
	public show(table: string): this {
		if (table && typeof table === 'string') {
			_changeVisibility = true;
			_query = `UPDATE ?? SET ${_pool.escapeId(table)}.${_pool.escapeId(_delColumnName)} = 1 `;
			_arrValues.push(table);
			_usedMethod = `show()`;
			return this;
		} else {
	...
	public innerJoin(table: string, staticTable: boolean = false): this {
		if (table && typeof table === 'string') {
			...
			*/
			...
			throw new Error('[Método innerJoin()]: El parámetro «table» debe ser un string y es requerido');
		}
	}
	public leftJoin(table: string, staticTable: boolean = false): this {
		if (table && typeof table === 'string') {
			...
			*/
			...
			throw new Error('[Método leftJoin()]: El parámetro «table» debe ser un string y es requerido');
		}
	}
	public rightJoin(table: string, staticTable: boolean = false): this {
		if (table && typeof table === 'string') {
			...
			*/
			...
			throw new Error('[Método rightJoin()]: El parámetro «table» debe ser un string y es requerido');
		}
	}
	...
	public orderBy(column: string, mode: OrderMode = 'ASC'): this {
		if (column && typeof column === 'string') {
			...
			throw new Error('[Método orderBy()]: El parámetro «column» debe ser un string y es obligatorio')
		}
	}
	public limit(startOrAmount: number, numRows: number | null = null): this {
		if (startOrAmount === undefined || typeof startOrAmount !== 'number') {
			throw new Error('[Método limit()]: Debe pasar por lo menos el número de filas que quiere limitar');
		}
